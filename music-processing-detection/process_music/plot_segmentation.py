...
# Segmentation demo found here: https://librosa.org/librosa_gallery/auto_examples/plot_segmentation.html.
# Removed all the x_coords=, y_coords= as these statements cause errors. Some other minor updates as well.
# - Leo
...
def compute_segments(song_file, show_outputs):
    #############################
    # First, we'll load in a song from first argument
    ...
    # Note - bound_times includes time of end of song.
    np.save("segment_times.npy", bound_times)
    np.save("segment_nums.npy", bound_segs)

    if show_outputs:
        # Print the segment change times and corresponding segment numbers.
        print(bound_times)
        print(bound_segs)
        # Show all the plots
        plt.show()
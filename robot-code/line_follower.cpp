...
void setup() {
  ...
  // Don't disable the lightbar ADC chips
...
void loop() {
  ...
  while (true) {
    ...
    target_v = 0.1; // Go forward slowly
    ...
    // Simple code to follow line by adjusting target_omega, not target_theta
    // If any of the middle three sensors are over line
    if (adc2_buf[3] < 400 || adc1_buf[3] < 400 || adc2_buf[2] < 400) {
      target_omega = 0; // Don't turn
    }
    // Else if any of the left five are over line
    else if (adc1_buf[6] < 400 || adc1_buf[5] < 400 || adc2_buf[5] < 400 || adc2_buf[4] < 400 || adc1_buf[4] < 400) {
      target_omega = 1; // Turn left
      ledcWriteNote(BUZZ_CHANNEL, NOTE_C, 4); // Beep beep
    }
    else if (adc1_buf[0] < 400 || adc1_buf[1] < 400 || adc2_buf[0] < 400 || adc2_buf[1] < 400 || adc1_buf[2] < 400) {
      target_omega = -1; // Turn right
      ledcWriteNote(BUZZ_CHANNEL, NOTE_G, 4);
    }
    else {
      target_omega = 0; // Don't turn
